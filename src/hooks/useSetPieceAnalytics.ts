...
function processEvents(
    ...
    const calcStats = (items: any[], type: 'throw_in' | 'corner' | 'free_kick'): SetPieceStats => {
        const successful = items.filter((e: any) => e.successful).length;
        return {
            type,
            total: items.length,
            successful,
            failed: items.length - successful,
            successRate: items.length > 0 ? Math.round((successful / items.length) * 100) : 0
        };
    };

    const overview: SetPieceStats[] = [
        calcStats(throwIns, 'throw_in'),
        calcStats(corners, 'corner'),
        calcStats(freeKicks, 'free_kick')
    ];
    ...
    playerMap.forEach(stats => {
        stats.throwIns.rate = stats.throwIns.total > 0
            ? Math.round((stats.throwIns.successful / stats.throwIns.total) * 100) : 0;
        stats.corners.rate = stats.corners.total > 0
            ? Math.round((stats.corners.successful / stats.corners.total) * 100) : 0;
        stats.freeKicks.rate = stats.freeKicks.total > 0
            ? Math.round((stats.freeKicks.successful / stats.freeKicks.total) * 100) : 0;
    });
...
export function useSetPieceAnalytics(
    ...
    const matchIds = Array.isArray(matchId) ? matchId : matchId ? [matchId] : [];

    return useQuery({
        queryKey: ['set-piece-analytics', matchIds, teamId, half],
        queryFn: async (): Promise<SetPieceAnalyticsData> => {
            if (matchIds.length === 0 || !teamId) throw new Error('Match ID(s) and Team ID required');

            const allEvents = await fetchEventsForMatches(matchIds);
            ...
            return processEvents(teamEvents);
        },
        enabled: matchIds.length > 0 && !!teamId
    });
}
